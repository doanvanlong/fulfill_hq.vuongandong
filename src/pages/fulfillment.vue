...
export default {
  ...
  methods: {
    ...
    async loginFlashShip(username_fs, password_fs) {
      return new Promise((resolve, reject) => {
        fetch('https://api.flashship.net/seller-api-v2/token', {
          method: 'POST',
          headers: {
    ...
    async addOrderFS(rawData, token) {
      return new Promise((resolve, reject) => {
        ...
        fetch('https://api.flashship.net/seller-api-v2/orders/shirt-add', requestOptions)
          .then((response) => {
            if (!response.ok) {
              throw new Error('Network response was not ok');
            }
    ...
    async orderDetailFs(orderId, token) {
      try {
        ...
        const response = await fetch(`https://api.flashship.net/seller-api-v2/orders/${orderId}`, requestOptions);
